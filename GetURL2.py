      ...
      if phychannelsList[0].__contains__('params'):
        for phychannelDict in phychannelsList:
          ...
          if urlResult != 'none' and channelDict['subTitle'] !='' :
            writeResult2(outFile,subTitle=channelDict['subTitle'],urlResult=urlResult,channelDict=channelDict, phychannelDict=phychannelDict)
      else:
...
def writeResult2(outFile,subTitle,urlResult,channelDict,phychannelDict):
    f2 = open(outFile, 'a')
    #subTitle = '#EXTINF:-1' + ' tvg-logo="' + re.sub('([0-9]{1,3}\.){3}[0-9]{1,3}:[0-9]{1,5}','192.168.6.249',channelDict['icon']) + '",' + channelDict['subTitle']
    subTitle = '#EXTINF:-1' + ' tvg-logo="' + channelDict['icon'] + '",' + channelDict['subTitle'] + '-' + phychannelDict['bitrateTypeName']
    f2.write(subTitle + '\n')
    #f2.write('http://192.168.6.1:4022/udp/' + urlResult.strip('rtp://|udp://') + '\n')
    f2.write(urlResult + '\n')
    f2.close()
